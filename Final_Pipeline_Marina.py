...
def wav2vec2(audio, sr, processor, model, hotwords=None, output_word_offsets=True, 
    ...
    end_offset = end_offset if end_offset else audio_length

    if start_offset < 0:
        print("start_offset is negative, setting it to 0")
        start_offset = 0
    if end_offset > audio_length:
        print("end_offset is too large, setting it to the end of the audio")
        end_offset = audio_length
    if start_offset > end_offset:
        print("start_offset is larger than end_offset, taking the entire audio")
        start_offset = 0
        end_offset = audio_length
...
def run_models(audio, target_duration_segment, min_duration_segment, overlap, reference_text, processor_phoneme, 
    ...
    if audio_length < target_duration_segment + min_duration_segment + 2: # no need to segment if the audio is too short (padding included)
        segmenter = False
    ...
    return transcriptions_segments, logits_segments, char_offsets_segments, transcription_asr_segmenter['text']
...
def is_followed_by_pads_and_same_value(new_prediction, i, pad_token_id):
    j = i + 1
    # Skip pad tokens
    while j < len(new_prediction) and new_prediction[j] == pad_token_id:
        j += 1
    # Check if we've reached the end of the sequence or if the next non-pad token is the same as new_prediction[i]
    return j == len(new_prediction) or new_prediction[j] == new_prediction[i]
...
def multiple_predictions(sub_logits_segment, processor_phoneme, threshold_diff, 
    ...
    if int(threshold_diff) == 0:
        threshold_diff_tuple = [(0, threshold_diff)]
    else:
        threshold_diff_tuple = [(i, i+1) for i in range(int(threshold_diff)-1)]
        threshold_diff_tuple.append((int(threshold_diff)-1, threshold_diff))

    # Iterate over each alternative prediction for each timeframe
    for min, max in threshold_diff_tuple:
        for j in range(1, k):
                ...
                if not ((top1_idx == pad_token_id and alt_idx == space_token_id) 
                    ...
                    if min<diff < max:
                        ...
                        for pred in predictions:
                            ...
                            if i > 0 and new_prediction[i] == new_prediction[i-1]:
                                continue
                            if i < len(new_prediction)-1 and new_prediction[i] == new_prediction[i+1]: 
                                continue
                            # if indices after i is a serie of pad tokens followed by same value as new_prediction[i] skip
                            if is_followed_by_pads_and_same_value(new_prediction, i, pad_token_id):
                                continue
...
def process_predictions(decoded_prediction_concat, target_phonemes, error_list, correct, results_decode):
...
def decode_phonetic_prediction_concat_error(all_decoded_predictions, target_phonemes, error_list, correct, results_decode):
    if len(all_decoded_predictions) > 2:
        for j in range(1, len(all_decoded_predictions) - 1):
            ...
            decoded_prediction_concat = [prev + curr + next 
                                         for prev in previous_pred_list
                                         for curr in current_pred_list
                                         for next in next_pred_list]
            correct, results_decode = process_predictions(decoded_prediction_concat, target_phonemes, error_list, correct, results_decode)
    elif len(all_decoded_predictions) == 2:
        previous_pred_list, current_pred_list = all_decoded_predictions
        decoded_prediction_concat = [prev + curr 
                                     for prev in previous_pred_list
                                     for curr in current_pred_list]
        correct, results_decode = process_predictions(decoded_prediction_concat, target_phonemes, error_list, correct, results_decode)
    elif len(all_decoded_predictions) == 1:
        decoded_prediction_concat = all_decoded_predictions[0]
        correct, results_decode = process_predictions(decoded_prediction_concat, target_phonemes, error_list, correct, results_decode)

    return correct, all_decoded_predictions, results_decode


def decode_phonetic_prediction_match(all_decoded_predictions, target_phonemes, error_list, correct, results_decode):
    """
    Decodes the phonetic predictions by concatenating previous, current, and next predictions,
    and checks for errors in the concatenated predictions.
    ...
    for i, targets_words in enumerate(target_phonemes):
        for item_preds in all_decoded_predictions:
            if any(error in item_preds for error in error_list[i]):
                correct[i] = 0
                break
            for target_word in targets_words:
                for item_pred in item_preds:
                    if not correct[i] and target_word == item_pred:
                        correct[i] = 1
                        results_decode[i] = [item_pred]
                        break
...
def pipeline_asr_function(audio, reference_text, model_asr, processor_asr, model_phoneme, processor_phoneme, 
    ...
    correct_match = [0] * len(reference_words)
    results_decode = [[]] * len(reference_words)
    results_decode_match = [[]] * len(reference_words)
    ...
    transcriptions_segments, logits_segments, char_offsets_segments, transcription_asr_segmenter = run_models(audio, target_duration_segment, min_duration_segment, 
    ...
    for logits_segment, char_offsets_segment in zip(logits_segments, char_offsets_segments):
        ...
        correct_match, _, results_decode_match = decode_phonetic_prediction_match(all_decoded_predictions, target_phonemes, 
                                                                                    error_list, correct_match, results_decode_match)
        all_decoded_predictions_whole.extend(all_decoded_predictions)

    return correct, results_decode, all_decoded_predictions_whole
...
def asr_pipeline():
    ...
    try:
        ...
        target_duration_segment = data.get('target_duration_segment', 4)
        min_duration_segment = data.get('min_duration_segment', 2)
        threshold_diff = data.get('threshold_diff', 1.5)
        max_pred_per_word = data.get('max_pred_per_word', 25)
        k = data.get('k', 10)
        top_k_is_all  = data.get(top_k_is_all, False)  # If top_k_is_all is True, set k to the length of the vocab
        ...
        if top_k_is_all:
            k = len(processor_phonemizer.tokenizer.get_vocab())
        # Run the pipeline on the audio file
        correct, results_decode, all_decoded_predictions_whole = pipeline_asr_function(
            audio=audio,
        ...
        response = {
            ...
            'all_decoded_predictions_whole': all_decoded_predictions_whole,	
        }
...
if __name__ == '__main__':
    ...
    parser.add_argument("--flask_port", help="The port of the flask app.", default=5000, type=int)
    ...
    parser.add_argument("--target_excel_path" ,type = str, default = 'Transcription_patients_IPA.xlsx')
